...
#  Instruction  |    Z   |   Trap    |   CondTrue   |    PCSel*   |   ASel*    |   BSel*    |   ImmSel*   |...
    PCcons = {"jmp": 1,"nextPC":0,"jalr":2,"trap":3,"beq0":0,"beq1":1,"bne0":1,"bne1":0,\
        "blt0":0,"blt1":1,"bge0":1,"bge1":0,"bltu0":0,"bltu1":1,"bgeu0":1,"bgeu1":0}
        ...
        with open('testvectorControlUnit.txt','w') as f:
            ...
            for a in testvector:
                ...
                if isa[a][1] == 'BRANCH':
                    ...
                    op = isa[a][2].lower()
                    pc = PCcons[op+str(z)] if(op == "beq" or op == "bne") else PCcons[op+str(condt)]               
                else:
                    pc = PCcons[isa[a][3]]
                    alu= alusel[isa[a][8]]
                ...
                logics = isa[a][9:]
                txt = "{:08X} {} {} {} {}\n".format(instr," ".join([str(z),str(trap),str(condt)])," ".join(sels)," ".join(logics),isa[a][2])
                f.write(txt)
