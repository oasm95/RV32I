...
def BP():
    N = 10000
    fsm = ((0,(0,1)),(0,(0,2)),(1,(1,3)),(1,(2,3)))
    initialstate = 0
    size = 256
    indexmask = size-1
    indexbits = int(log2(size))
    currentstate = [initialstate]*size
    targetTable = [0]*size
    tags = [0]*size
    jmptypes = [0]*size
    
    with open('testvectorBP.txt','w') as f:
        wen= 1
        wasmatch = 0
        Raddr = 0
        for Waddr in range(size):
            isjal = 1 if( Waddr % 10 ==0 ) else 0
            takenjmp = random.randint(0,1)
            wtarget = random.randint(0,0xffffffff)
            
            tableindex = indexmask & (Waddr)
            tag = Waddr >> (indexbits) 
            
            tags[tableindex] = tag
            jmptypes[tableindex] = isjal
            targetTable[tableindex] = wtarget
            nextstate = fsm[initialstate][1]
            currentstate[tableindex] = nextstate[takenjmp]
            
            f.write(f'{isjal} {wen} {takenjmp} {wasmatch} {Raddr:08x} {Waddr<<2:08x} {wtarget:08x} {0:08x} 0 0\n')
        
        for _ in range(N):
            prevwrite = Waddr
            isjal = 1 if(_ % 10 ==0 ) else 0
            wen = random.randint(0,1)
            takenjmp = random.randint(0,1)        
            Waddr = random.randint(0,0x3ffff)
            wtarget = random.randint(0,0xffffffff)
            #update table
            wasmatch = 1 if(_ % 2) else 0
            Raddr = prevwrite if (wasmatch) else random.randint(0,0x3ffff)
            
            tableindex = indexmask & (Raddr>>2)
            tag = Raddr >> (indexbits+2)        
            
            match = 0
            rtarget = targetTable[tableindex]
            jalread = jmptypes[tableindex]
            if tags[tableindex] == tag:
                state = currentstate[tableindex]
                prediction = fsm[state][0]
                match = 1
                if jalread:
                    prediction = 1
            else:
            ...
            tableindex = indexmask & (Waddr>>2)
            tag = Waddr >> (indexbits+2)     
            
            state = currentstate[tableindex]
            nextstate = fsm[state][1]
            ...
            f.write(f'{isjal} {wen} {takenjmp} {wasmatch} {Raddr:08x} {Waddr:08x} {wtarget:08x} {rtarget:08x} {prediction} {match}\n')
