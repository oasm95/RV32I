...
architecture rtl of CSRFile is
    ...
    signal mscratch: std_logic_vector(BUS_Width-1 downto 0);
...
-- CSR Write processes
    ...
    mscratch_write: process(clk)
    begin
        if (rising_edge(clk)) then
            if (CSR_Wen = '1') and (CSR_addr = mscratch_addr) and (CSR_Ret /= '1') and (CSR_Wen_traps /= '1')then
                mscratch <= CSR_Wdata(BUS_Width-1 downto 0);
            end if;
        end if;
    end process;
    ...
    csr_read: process(CSR_addr,mcause,mstatus,mepc,mvect,mie,mip,mscratch)
